...
class Attention(nn.Module):
    """HAViT attention with blending in probability space (after softmax)."""

    def __init__(self, dim, heads=8, dim_head=64, dropout=0.0, alpha=0.9):
        super().__init__()
    ...
    def forward(self, x, h=None):
        ...
        attn_cur = torch.softmax(raw, dim=-1)
        h_prev = torch.ones_like(attn_cur) / attn_cur.shape[-1] if h is None else h
        attn = self.alpha * attn_cur + (1.0 - self.alpha) * h_prev
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0.0, alpha=0.9):
        super().__init__()
        self.norm = nn.LayerNorm(dim)
        self.layers = nn.ModuleList(
            [
                nn.ModuleList(
                    [
                        Attention(
                            dim, heads=heads, dim_head=dim_head, dropout=dropout, alpha=alpha
                        ),
...
class havit_post_softmax(nn.Module):
